...
import {
  getAllChannels,
  getChannel,
  getLastMessagesInChannel
} from "@/db/channels"

export const channelsApi = (app: Elysia) =>
  app
    .use(kitPlugin)
    .get(
      "/channels/:id",
      async (ctx): Promise<ChannelRow | null> => {
    ...
    .get(
      ...
      async (ctx): Promise<MessageRow[]> => {
        ...
        const messageRes = await getLastMessagesInChannel(
          ctx.store.kit,
          ctx.params.id,
          last,
          before
